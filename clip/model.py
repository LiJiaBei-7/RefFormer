...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        pos_embed = self.resize_pos_embed(pos_embed, (H, W)).reshape(-1,C,H,W)  # NC(HW) -> NCHW 
        x = x + pos_embed
        x = x.flatten(start_dim=2).permute(2, 0, 1)  # NCHW -> (HW)NC
        x = torch.cat([x.mean(dim=0, keepdim=True), x], dim=0)  # (HW+1)NC
        # x = x + pos_embed[:, None, :].to(x.dtype)  # (HW+1)NC
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        self.positional_embedding = nn.Parameter(torch.empty(self.context_length, transformer_width))   # torch.Size([77, 512])
...
def build_model(state_dict: dict, resolution=None):
    ...
    if vit:
        ...
        pos_embed_reshaped = interpolate_pos_embed(state_dict['visual.positional_embedding'].unsqueeze(dim=0),
                                                       num_patches=num_patches, num_extra_tokens=1)  
        state_dict['visual.positional_embedding'] = pos_embed_reshaped.squeeze(dim=0)
        ...
        # num_patches = image_resolution // 32
        # pos_embed_reshaped = interpolate_pos_embed(state_dict['visual.attnpool.positional_embedding'].unsqueeze(dim=0),
        #                                                num_patches=num_patches, num_extra_tokens=1)
        # print(pos_embed_reshaped.shape)
        # exit()  
        # state_dict['visual.attnpool.positional_embedding'] = pos_embed_reshaped.squeeze(dim=0)
