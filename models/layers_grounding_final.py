...
class HA_swin(nn.Module):
    def __init__(self,
                 in_channels=[256, 512, 512],
                 out_channels=[256, 512, 1024],
                 stride = [2, 1, 2], # [1, 1, 1] for vit
                 d_model = 512, nhead = 8):
        super(HA_swin, self).__init__()
        ...
        self.f3_proj = conv_layer(in_channels[0], out_channels[1], kernel_size=4, padding=0, stride=4)
        self.f4_proj = conv_layer(in_channels[1], out_channels[1], kernel_size=2, padding=0, stride=2)
        self.f5_proj = deconv_layer(in_channels[2], out_channels[1], kernel_size=1, padding=0, stride=1)
        # aggregation
    ...
    def forward(self, imgs, state):
        ...
        # v3, v4, v5 = imgs['s2'], imgs['s3'], imgs['s4']
        v3, v4, v5 = imgs
        txt = state.unsqueeze(0)

        # torch.Size([4, 6400, 256])
        # torch.Size([4, 1600, 512])
        # torch.Size([4, 400, 512])

        H = W = int(v3.size(1) ** 0.5)
        v3 = self.f3_proj(v3.reshape(v3.size(0), H, W, v3.size(-1)).permute(0,-1,1,2))
        H = W = int(v4.size(1) ** 0.5)
        v4 = self.f4_proj(v4.reshape(v4.size(0), H, W, v4.size(-1)).permute(0,-1,1,2))
        
        H = W = int(v5.size(1) ** 0.5)
        v5 = self.f5_proj(v5.reshape(v5.size(0), H, W, v5.size(-1)).permute(0,-1,1,2))
...
class GA(nn.Module):
    def __init__(self,
        ...
        # self.agg = Aggregation(d_model=d_model,
        #                             nhead=nhead,
        #                             dim_feedforward=dim_ffn,
        #                             dropout=dropout)
        
        self.layers = nn.ModuleList([
            SGALayer(d_model=d_model,
                                    nhead=nhead,
                                    dim_feedforward=dim_ffn,
    ...
    def forward(self, reg_token, vis, txt, pad_mask, query_pos=None, return_attn=False):
        ...
        # vis = self.agg(reg_token, vis, txt, vis_pos, txt_pos, pad_mask, query_pos, return_attn)
...
class SGALayer(nn.Module):
    ...
    def forward(self, reg_token, vis, txt, vis_pos, txt_pos, pad_mask, query_pos=None, return_attn=False):
        ...
        q = src2.clone()
        q[:n_q] = self.with_pos_embed(q[:n_q], query_pos)
        q[n_q:] = self.with_pos_embed(q[n_q:], vis_pos)
        k = self.with_pos_embed(src2[n_q:], vis_pos)
        
        src3, attn = self.self_attn2(q, k, value=src2[n_q:])
...
class QFormer(nn.Module):
    def __init__(self, d_model, nhead,):
        ...
        self.gate_token = nn.Embedding(1, d_model)
        self.gate_token1 = nn.Embedding(1, d_model)
...
#         self.ln = nn.LayerNorm(d_model)
        ...
        # new add
        # self.self_reg = SelfAttention(d_model=d_model, nhead=nhead, num_layers=1)

        self.mlp_gate_v = MLP(d_model, d_model, 1, 3)
        self.mlp_gate_t = MLP(d_model, d_model, 1, 3)
                
    def forward(self, reg_token, vis, txt, pos_query=None, return_attn=False):
        
        txt_gate_token, vis_gate_token = self.gate_token.weight.unsqueeze(1).repeat(1, vis.size(1), 1), self.gate_token.weight.unsqueeze(1).repeat(1, vis.size(1), 1)
 
        n_q = reg_token.size(0)

        # new add
        # reg_token = self.self_reg(reg_token)
    
        tgt = torch.cat([reg_token, vis_gate_token, vis], dim=0)
        tgt, txt = self.cross_v(tgt, txt, query_pos=pos_query), self.cross_t(torch.cat([txt_gate_token, txt], dim=0), vis)
                
        tgt, attn = self.self_v(tgt, tgt[n_q+1:], query_pos=pos_query, return_attn=True)
        txt = self.self_t(txt)

        queries, vis_gate_token, vis = tgt[:n_q], tgt[n_q:n_q+1], tgt[n_q+1:]
        txt_gate_token, txt = txt[:1], txt[1:]

        vis_gate_token, txt_gate_token = self.mlp_gate_v(vis_gate_token), self.mlp_gate_t(txt_gate_token)
        vis, txt = torch.sigmoid(vis_gate_token) * vis, torch.sigmoid(txt_gate_token) * txt
