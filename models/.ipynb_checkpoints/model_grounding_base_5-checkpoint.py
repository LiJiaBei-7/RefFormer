...
from models.Bridger_5 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    ...
    def set_aux_criterion_roi(self, output):
        ...
        for i, (feat,reg) in enumerate(zip(features, reg_tokens)):
            ...
            loss = F.l1_loss(self.bridger.aux_heads[i](reg.squeeze(0)), output, reduction='mean')
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        vis, word, state, reg_tokens, feature_ls, attn_ls = self.bridger(image, text_ids, self.backbone)
        
        reg_token = reg_tokens[-1] #(1,B,D)
       
        fq = self.neck(vis, state) # torch.Size([32, 512, 16, 16])
        
        fq, attn = self.decoder(reg_token, fq, word, pad_mask=pad_mask, query_pos=self.bridger.reg_token.weight, return_attn=True) # torch.Size([32, 512, 257])
        ...
        #     with h5py.File('attn_data/Bridger_3_GA2.hdf5', 'a') as f:
        ...
        # if torch.distributed.get_rank() == 0:
        #     with h5py.File('attn_data/decoder_attn/Bridger_3_GA2.hdf5', 'a') as f:
        #         for i in range(len(idx)):
        #             if str(idx[i].item()) not in f.keys():
        ...
        if training:
            ...
            output.update(dict(features=feature_ls))
