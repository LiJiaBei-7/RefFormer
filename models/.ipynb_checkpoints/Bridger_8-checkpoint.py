...
from .layers_8 import conv_layer, deconv_layer, InteractorT, Interactor, MLP, CrossAttention, SelfAttention, BridgerFormer
...
class Bridger_ViT_1(nn.Module):
    def __init__(self,
        ...
        self.n_reg = num_reg
        self.reg_token = nn.Parameter(torch.zeros(self.n_reg, 1, d_img))
        ...
        for i in range(num_stages):
            ...
            self.hidden_decoder.append(CrossAttention(d_model=d_img, nhead=nhead, num_layers=2))
    ...
    def forward(self, img, text, backbone):
        ...
        for i in range(self.num_layers):
            if i not in [3,9,11]: #[3,5,7,9,11]:
                ...
                v, t = self.bridgeformer[stage_i](vis=v, txt=t)
        ...
        txt_outs.append(txt.transpose(0,1))

        # forward
        output = vis_outs, txt, state, txt_outs
