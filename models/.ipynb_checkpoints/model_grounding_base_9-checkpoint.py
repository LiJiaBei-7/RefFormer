...
from models.layers_9 import GA, MLP, CrossAttention, HA #HA2 as HA
from models.Bridger_9 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.de_pos = nn.Embedding(1, dim_ffn)
    ...
    def set_aux_criterion(self, output):
        ...
        for i, rt in enumerate(reg_tokens):
            rt = rt.permute(1,0,-1).flatten(1)
            rt = self.bridger.aux_proj[i](rt)
    ...
    def set_criterion(self, output):
        ...
        losses['loss_aux'] = torch.tensor(0.).cuda().detach() #self.set_aux_criterion(output)
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        bs = state.size(0)
        tgt = torch.ones(1, bs, 512).cuda()

        fq = self.neck(vis, reg_tokens)

        fq, attn = self.decoder(tgt, fq, word, pad_mask=pad_mask, query_pos=self.de_pos.weight, return_attn=True) # torch.Size([32, 512, 257])
