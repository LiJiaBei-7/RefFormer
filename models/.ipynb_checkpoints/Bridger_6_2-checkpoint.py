...
from .layers_6 import conv_layer, deconv_layer, InteractorT, Interactor, MLP, CrossAttention, SelfAttention, BridgerFormer
...
class Bridger_ViT_1(nn.Module):
    ...
    def forward(self, img, text, backbone):
        ...
        # reg_token = None
        bs = txt.size(1)  
        _reg = torch.zeros(self.n_reg, bs, self.d_model).cuda()
        reg = self.reg_token.expand(-1,bs,-1)
        ...
        for i in range(self.num_layers):
            if i not in [3,5,7,9,11]:
                ...
                tgt = torch.ones(self.n_reg, bs, self.d_model).cuda()
                # reg, tgt1, v, t, attn = self.bridgeformer[stage_i](tgt, self.reg_token.expand(-1,bs,-1)+reg, v, t, pos_query=None, return_attn=True)
                reg, tgt1, v, t, attn = self.bridgeformer[stage_i](tgt, reg+_reg, v, t, pos_query=None, return_attn=True)
        ...
        # forward
        output = vis_outs, txt, state, reg_tokens, condition_ls, attn_ls
