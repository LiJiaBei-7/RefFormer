...
from models.layers_6_2_4 import GA, MLP, CrossAttention, HA #HA2 as HA
from models.Bridger_6_2_4 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    ...
    def set_aux_criterion(self, output):
        ...
        for i, rt in enumerate(reg_tokens):
            ...
            loss_aux_item += f'{i}-layer is {(loss_aux_l1 + loss_aux_giou).item():.2f} | '
        return loss_aux 
    ...
    def set_criterion(self, output):
        ...
        losses['loss_aux'] = torch.tensor(0.).cuda().detach() #self.set_aux_criterion(output) * 0.1 #torch.tensor(0.).cuda().detach() #self.set_aux_criterion(output)
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        fq, attn = self.decoder(reg+tgt, fq, word, pad_mask=pad_mask, query_pos=None, return_attn=True) # torch.Size([32, 512, 257])
        
        reg_token = fq.squeeze(0)
        pred_box = self.bbox_embed(reg_token).sigmoid() # torch.Size([32, 4])
        
        # if torch.distributed.get_rank() == 0:
        #     with h5py.File('attn_data/base_6_2_4_aux01.hdf5', 'a') as f:
        #         for i in range(len(idx)):
        #             if str(idx[i].item()) not in f.keys():
        #                 _attn = torch.cat([attn_ls[j][i] for j in range(len(attn_ls))], dim=0)
        ...
        # if torch.distributed.get_rank() == 0:
        #     with h5py.File('attn_data/decoder_attn/base_6_2_4_aux01.hdf5', 'a') as f:
        #         for i in range(len(idx)):
        #             if str(idx[i].item()) not in f.keys():
