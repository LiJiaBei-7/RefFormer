...
class HA(nn.Module):
    def __init__(self,
        ...
        self.s3_proj = nn.Linear(ladder_dim, out_channels[1]) 
        self.s4_proj = nn.Linear(ladder_dim, out_channels[1])
        ...
        self.fuse_proj3 = nn.Linear(out_channels[1], out_channels[1])
        self.fuse_proj4 = nn.Linear(out_channels[1], out_channels[1])
        # aggregation
    ...
    def forward(self, imgs, state, feats):
        ...
        s3, s4 = self.s3_proj(s3), self.s4_proj(s4)
        
        # fusion v3 
        b, c, h, w = v3.shape
        v3 = v3.reshape(b, c, -1).permute(2, 0, 1) # b, c, h, w -> b, c, hw -> hw, b, c
        # print(v3.shape, txt.shape)
        txt = self.fuse_proj3(s3*txt) + txt
        # txt = self.fuse_proj3(torch.cat([s3,txt], dim=-1)) * s3 + txt
        ...
        txt = self.fuse_proj4(s4*txt) + txt
        # txt = self.fuse_proj3(torch.cat([s4,txt], dim=-1)) * s4 + txt
...
class HA1(nn.Module):
    def __init__(self,
        ...
        self.fuse_proj3 = nn.Linear(out_channels[1]*2, out_channels[1])
        self.fuse_proj4 = nn.Linear(out_channels[1]*2, out_channels[1])
        # aggregation
        self.aggr = conv_layer(3 * out_channels[1], out_channels[1], 1, 0)
        self.coordconv = nn.Sequential(
            CoordConv(out_channels[1], out_channels[1], 3, 1),
    ...
    def forward(self, imgs, state, feats):
        ...
        v3 = v3.reshape(b, c, -1).permute(2, 0, 1) # b, c, h, w -> b, c, hw -> hw, b, c
        # print(v3.shape, txt.shape)
        v3 = self.fuse_proj3(torch.cat([v3, s3.repeat(v3.size(0),1,1)], dim=-1)) + v3
        fq3 = self.fusion3(v3, txt)        
        fq3 = fq3.permute(1, 2, 0).reshape(b, c, h, w)
        # fusion v4 
        b, c, h, w = v4.shape
        v4 = v4.reshape(b, c, -1).permute(2, 0, 1) # b, c, h, w -> b, c, hw -> hw, b, c     
        # v4 = self.downsample(v4)
        v4 = self.fuse_proj4(torch.cat([v4, s4.repeat(v4.size(0),1,1)], dim=-1)) + v4
        fq4 = self.fusion4(v4, txt)        
        fq4 = fq4.permute(1, 2, 0).reshape(b, c, h, w)
        # fusion v5 
        b, c, h, w = v5.shape
