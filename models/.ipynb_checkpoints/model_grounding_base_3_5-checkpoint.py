...
from models.layers_3_5 import HA, GA2 as GA, MLP, CrossAttention
...
class Model(nn.Module):
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        if torch.distributed.get_rank() == 0:
            with h5py.File('attn_data/Bridger_3_5_379_noavg.hdf5', 'a') as f:
        ...
        if torch.distributed.get_rank() == 0:
            with h5py.File('attn_data/decoder_attn/Bridger_3_5_379_noavg.hdf5', 'a') as f:
                for i in range(len(idx)):
                    if str(idx[i].item()) not in f.keys():
