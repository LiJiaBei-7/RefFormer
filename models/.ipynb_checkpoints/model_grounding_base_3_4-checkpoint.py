...
from models.layers_3 import HA, GA2 as GA, MLP, CrossAttention
from models.Bridger_3 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.decoder_reg = nn.Embedding(self.bridger.n_reg, dim_ffn)
    ...
    def set_aux_criterion(self, output):
        ...
        return loss_aux / len(reg_tokens)
    ...
    def set_criterion(self, output):
        ...
        losses['loss_aux'] = self.set_aux_criterion(output)
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        vis, word, state, reg_tokens, attn_ls = self.bridger(image, text_ids, self.backbone)
        
        reg_token = reg_tokens[-1] #(1,B,D)
       
        fq = self.neck(vis, state) # torch.Size([32, 512, 16, 16])
        
        fq, attn = self.decoder(reg_token.detach(), fq, word, pad_mask=pad_mask, query_pos=self.bridger.reg_token.weight.detach() + self.decoder_reg.weight, return_attn=True) # torch.Size([32, 512, 257])
        ...
        if torch.distributed.get_rank() == 0:
            with h5py.File('attn_data/Bridger_3_GA2_detach_add_new_pos.hdf5', 'a') as f:
        ...
        if torch.distributed.get_rank() == 0:
            with h5py.File('attn_data/decoder_attn/Bridger_3_GA2_detach_add_new_pos.hdf5', 'a') as f:
                for i in range(len(idx)):
                    if str(idx[i].item()) not in f.keys():
