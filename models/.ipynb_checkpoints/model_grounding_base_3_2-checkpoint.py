...
from models.Bridger_3_2 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        if torch.distributed.get_rank() == 0:
            with h5py.File('attn_data/Bridger_3_GA2_condition.hdf5', 'a') as f:
        ...
        if torch.distributed.get_rank() == 0:
            with h5py.File('attn_data/decoder_attn/Bridger_3_GA2_condition.hdf5', 'a') as f:
                for i in range(len(idx)):
                    if str(idx[i].item()) not in f.keys():
