...
from models.layers_8 import GA, MLP, CrossAttention, Decoder
from models.Bridger_8 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        vis, word, state, txt = self.bridger(image, text_ids, self.backbone)
        
        fq = self.decoder(self.query, vis, txt, pad_mask=pad_mask, return_intermediate=True)
        reg_token = fq[-1].squeeze(0)
...
#                         f.create_dataset(f'pred_{idx[i].item()}', data=pred_box[i].detach().cpu())
        ...
        if training:
            output.update(dict(reg_tokens=fq[:-1]))
