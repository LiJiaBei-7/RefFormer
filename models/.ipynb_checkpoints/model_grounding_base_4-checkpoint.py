...
from models.layers_4 import GA, MLP, CrossAttention, HA #HA2 as HA
from models.Bridger_4 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        fq, attn = self.decoder(tgt, fq, word, pad_mask=pad_mask, query_pos=None, return_attn=True) # torch.Size([32, 512, 257])
    
        reg_token = fq[...,0]
        pred_box = self.bbox_embed(reg_token).sigmoid() # torch.Size([32, 4])
        
#         if torch.distributed.get_rank() == 0:
#             with h5py.File('attn_data/Bridger_4_notaddreg_512.hdf5', 'a') as f:
...
#         if torch.distributed.get_rank() == 0:
#             with h5py.File('attn_data/decoder_attn/Bridger_4_notaddreg_512.hdf5', 'a') as f:
#                 for i in range(len(idx)):
#                     if str(idx[i].item()) not in f.keys():
