...
from .layers_6_3 import conv_layer, deconv_layer, InteractorT, Interactor, MLP, CrossAttention, SelfAttention, BridgerFormer
...
class Bridger_ViT_1(nn.Module):
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            self.linear_reg.append(MLP(d_model, int(d_txt/2), d_txt, 2))
    ...
    def forward(self, img, text, backbone):
        ...
        reg_pos = self.reg_pos.weight.unsqueeze(1).repeat(1,bs,1)
        attn_ls = []
        _t = _v = None
        for i in range(self.num_layers):
            if i not in [3,5,7,9,11]:
                ...
                tgt = torch.ones(self.n_reg, bs, self.d_model).cuda()
                reg, v, t, attn = self.bridgeformer[stage_i](tgt, v, t, pos_query=reg_pos, return_attn=True)
                v = self.linear2_v[stage_i](v)
                t = self.linear2_t[stage_i](t)
                reg = self.linear_reg[stage_i](reg)
