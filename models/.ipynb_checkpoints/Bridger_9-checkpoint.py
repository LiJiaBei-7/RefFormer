...
from .layers_9 import conv_layer, deconv_layer, InteractorT, Interactor, MLP, CrossAttention, SelfAttention, BridgerFormer, Former
...
class Bridger_ViT_1(nn.Module):
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            self.former.append(Former(d_txt, nhead))
            self.reg_token.append(nn.Embedding(self.n_reg, d_txt).cuda())
            ...
            self.aux_proj.append(MLP(d_txt*num_reg, d_txt, d_txt, 1))
    ...
    def forward(self, img, text, backbone):
        ...
        for i in range(self.num_layers):
            if i not in [3,5,7,9,11]:
                ...
                v, t = self.bridgeformer[stage_i](v, t)
                v = self.linear2_v[stage_i](v)
                t = self.linear2_t[stage_i](t)
                
                if i in [3,9,11]:
                    tgt = self.reg_token[stage_i].weight.unsqueeze(1).repeat(1,bs,1)
                    query, attn = self.former[stage_i](tgt,v,t, return_attn=True)
                    reg_tokens.append(query)  # (1,B,D)
                    attn_ls.append(attn)
