...
from .layers_3 import conv_layer, deconv_layer, InteractorT, Interactor, MLP, CrossAttention, SelfAttention
from .Qformer import Qformer_2 as Qformer
...
class Bridger_ViT_1(nn.Module):
    def __init__(self,
        ...
        self.con_token = nn.Embedding(1, d_txt)
        self.reg_token = nn.Embedding(1, d_txt)
        self.aux_heads = MLP(d_txt, d_txt, 4, 3)
        ...
        for i in range(num_stages):
            ...
            self.Qformer.append(Qformer(hidden_size=[d_txt, d_img[i]], nhead=nhead, selfattn_layer=2, crossattn_layer=2))
    ...
    def forward(self, img, text, backbone):
        ...
        con_embed = self.con_token.weight.unsqueeze(1).repeat(1, bs, 1)
        ...
        for i in range(self.num_layers):
            if (i+1)%4 != 0:
                ...
                tgt, attn = self.Qformer[stage_i](con_embed, tgt, v+vis[1:, :, :], t+txt, query_pos=reg_embed, return_attn=True) # 1,B,D
