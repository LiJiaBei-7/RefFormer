...
class HA1(nn.Module):
    def __init__(self,
        ...
        self.fusion = InteractorT(d_model=d_model, nhead=nhead)   
        ...
        self.att = CrossAttention(d_model=out_channels[1], nhead=nhead, num_layers=1)
    ...
    def forward(self, imgs, state):
        ...
        fq = torch.cat([v3, v4, v5], dim=1)
        fq = self.aggr(fq)
        fq = self.coordconv(fq)
        fq = fq.flatten(-2).permute(-1,0,1)
        fq = self.att(fq, txt)

        n,b,c = fq.size()
        h=w=int(n**0.5)
        fq = fq.permute(1,-1,0).reshape(b, c, h, w)
...
class HA2(nn.Module):
    def __init__(self,
        ...
        self.coordconv = nn.Sequential(
            CoordConv(out_channels[1], out_channels[1], 3, 1),
            conv_layer(out_channels[1], out_channels[1], 3, 1))
        self.att = CrossAttention(d_model=out_channels[1], nhead=nhead, num_layers=2)
...
class GA(nn.Module):
    ...
    def forward(self, reg_token, vis, txt, pad_mask, query_pos=None, return_attn=False):
        ...
        # forward
        intermediate = []
        output = torch.cat((reg_token, vis), dim=0)
        # query_pos = query_pos.unsqueeze(1).repeat(1,B,1)
        for layer in self.layers:
            if return_attn:
                output, attn = layer(output, vis, txt, vis_pos, txt_pos, pad_mask, query_pos, return_attn)
            else:
                output = layer(output, vis, txt, vis_pos, txt_pos, pad_mask, query_pos, return_attn)
        ...
        if self.norm is not None:
            ...
            if self.return_intermediate:
                ...
                # b, 512, HW
                return output if not return_attn else output, attn
...
class SGALayer_1(nn.Module):
    ...
    def forward(self, src, vis, txt, vis_pos, txt_pos, pad_mask, query_pos=None, return_attn=False):
        '''
            vis: 26*26, b, 512
            txt: L, b, 512
            vis_pos: 26*26, 1, 512
        ...
        if return_attn:
            ...
            src3 = self.self_attn2(q, k, value=src2[1:], )[0]
...
class ConditionBridgerFormer(nn.Module):
...
#         self.ln = nn.LayerNorm(d_model)
    ...
    def forward(self, queries, reg_token, vis, txt, pos_query=None, return_attn=False):
        ...
        reg, vis = tgt[:n_q], tgt[n_q:]
        
        src = reg * queries
        src = torch.cat([src, vis], dim=0)
        
        tgt, attn = self.self_v(src, vis, query_pos=pos_query, return_attn=True)
        txt = self.self_t(txt)
        queries, vis = tgt[:n_q], tgt[n_q:]
        
        if return_attn:
            return reg, queries, vis, txt, attn
        else:
            return reg, queries, vis, txt
