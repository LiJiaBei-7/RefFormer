...
class GA(nn.Module):
    ...
    def forward(self, tgt, reg_token, vis, txt, pad_mask, query_pos=None, return_attn=False):
        '''
            vis: b, 512, h, w
            txt: b, L, 512
            pad_mask: b, L
        ...
        for layer in self.layers:
            if return_attn:
                tgt, reg_token, vis, attn = layer(tgt, reg_token, vis, txt, vis_pos, txt_pos, pad_mask, query_pos, return_attn)
            else:
                tgt, reg_token, vis = layer(tgt, reg_token, vis, txt, vis_pos, txt_pos, pad_mask, query_pos, return_attn)
            # output = layer(output, txt, vis_pos, txt_pos, pad_mask)
            if self.return_intermediate:
                # HW, b, 512 -> b, 512, HW
                intermediate.append(self.norm(tgt).permute(1, 2, 0))

        if self.norm is not None:
            # HW, b, 512 -> b, 512, HW
            tgt = self.norm(tgt).permute(1, 2, 0)
            if self.return_intermediate:
                intermediate.pop()
                ...
                # b, 512, HW
                return tgt if not return_attn else tgt, attn
...
class SGALayer_1(nn.Module):
    ...
    def forward(self, tgt, reg_token, vis, txt, vis_pos, txt_pos, pad_mask, query_pos=None, return_attn=False):
        '''
            vis: 26*26, b, 512
            txt: L, b, 512
            vis_pos: 26*26, 1, 512
        ...
        if return_attn:
            return tgt, reg_token, vis, attn
        else:
            return tgt, reg_token, vis
