...
from models.Bridger_6_2 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        vis, word, state, reg_tokens, condition_ls, attn_ls = self.bridger(image, text_ids, self.backbone)
        
        # tgt = self.tgt_proj(reg_tokens[-1])
        bs = state.size(0)
#         self.bridger.reg_token.expand(-1, bs, -1) 
        reg = self.reg_proj(condition_ls[-1]) 
        ...
        #     with h5py.File('attn_data/bridger_6_2_vit16.hdf5', 'a') as f:
        ...
        # if torch.distributed.get_rank() == 0:
        #     with h5py.File('attn_data/decoder_attn/bridger_6_2_vit16.hdf5', 'a') as f:
        #         for i in range(len(idx)):
        #             if str(idx[i].item()) not in f.keys():
