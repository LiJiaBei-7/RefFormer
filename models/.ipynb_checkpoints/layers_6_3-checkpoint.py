...
class HA(nn.Module):
    def __init__(self,
        ...
        self.reg3_proj = nn.Linear(in_channels[2], out_channels[1]) 
        self.reg4_proj = nn.Linear(in_channels[2], out_channels[1]) 
        self.reg5_proj = nn.Linear(in_channels[2], out_channels[1])   
        ...
        self.f5_proj = deconv_layer(in_channels[0], out_channels[1], stride[2], 0, stride[2])
        # aggregation
    ...
    def forward(self, imgs, states):
        ...
        v4 = self.f4_proj(v4)
        v5 = self.f5_proj(v5)
        s3 = self.reg3_proj(states[0])
        s4 = self.reg4_proj(states[1])
        s5 = self.reg5_proj(states[2])
...
class BridgerFormer(nn.Module):
    ...
    def forward(self, reg_token, vis, txt, pos_query=None, return_attn=False):
        ...
        tgt[:n_q] = tgt[:n_q] + pos_query
        tgt, txt = self.cross_v(tgt, txt), self.cross_t(txt, vis)
        
        q = tgt.clone()
        q[:n_q] = q[:n_q] + pos_query
        tgt, attn = self.self_v(q, tgt[n_q:], return_attn=True)
        txt = self.self_t(txt)
