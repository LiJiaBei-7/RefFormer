...
from models.Bridger_6_2_2 import Bridger_ViT_1 as Bridger_VL_1
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.reg_proj2 = nn.Linear(dim_ffn*2, dim_ffn)
    ...
    def forward(self, image, text_ids, targets=None, idx=None, text=None, epoch=None, training=False):
        ...
        vis_cls, vis, word, state, reg_tokens, attn_ls = self.bridger(image, text_ids, self.backbone)
        
        # tgt = self.tgt_proj(reg_tokens[-1])
        bs = state.size(0)
        reg = self.reg_proj(reg_tokens[-1]) 
        reg = self.reg_proj2(torch.cat([reg,vis_cls.transpose(0,1)], dim=-1))
        ...
        pred_box = self.bbox_embed(reg_token.squeeze(0)).sigmoid() # torch.Size([32, 4])
