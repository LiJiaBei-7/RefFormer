...
class Fusion(nn.Module):
    def __init__(self, d_model, bias=False):
        ...
        self.fusion = nn.Sequential(
            nn.Conv2d(d_model*2, d_model, 3, padding=1, bias=bias),
            nn.BatchNorm2d(d_model),
            nn.ReLU(),
            nn.Conv2d(d_model, d_model, 3, padding=1, bias=bias),
            nn.BatchNorm2d(d_model),
            nn.ReLU(),
        )
    def forward(self, in_1, in_2):
        ...
        H = int(N ** 0.5)
        W = N // H 
        in_1 = in_1.permute(1,2,0).reshape(B,C,H,W)
        in_2 = in_2.permute(1,2,0).reshape(B,C,H,W)
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, query, vis, txt, vis_pos, txt_pos, pad_mask):
        n_q = query.size(0)
    
        src = torch.cat([query, vis], dim=0)
        src = self.norm1(src)
        q = src.clone()
        q[n_q:] = self.with_pos_embed(q[n_q:], vis_pos)
        src2 = self.attn1(query=q, key=self.with_pos_embed(txt, txt_pos),value=txt, 
                    attn_mask=None,key_padding_mask=pad_mask)[0]
        ...
        q = src2.clone()
        q[n_q:] = self.with_pos_embed(q[n_q:], vis_pos)
        src3 = self.attn2(query=q, key=src2, value=src2)[0]
        src3 = self.norm3(src3)
        ...
        query, vis = src[:n_q], src[n_q:]
        
        return query, vis


class Decoder(nn.Module):
    def __init__(self, num_layer, d_model, nhead=8):
        super().__init__()
        ...
        for i in range(num_layer):
            self.layers.append(DecoderLayer(d_model=d_model, nhead=nhead))
            dimIn = 768
            if i == num_layer-1:
                dimIn = d_model
            self.projs.append(nn.Linear(dimIn, d_model))
    ...
    def forward(self, query, vis_outs, txt_outs, pad_mask, return_intermediate=False):
        vis = vis_outs[0]
        N,B,C = vis.shape
        H = int(N ** 0.5)
        W = N // H 
        L, _, D = txt_outs[0].shape
        vis_pos = self.pos2d(D, H, W).cuda()
        txt_pos = self.pos1d(D, L).cuda()
        query_ls = []
        query = query.unsqueeze(1).expand(-1,B,-1)
        # vis = vis.permute(0, 2, 1).reshape(B, C, H, W)
        for i, (layer, proj, vis) in enumerate(zip(self.layers[::-1], self.projs[::-1], vis_outs[::-1])):
            vis = proj(vis)
            if i != 0:
                vis = vis + _vis
            query, _vis = layer(query, vis, txt_outs[-1], vis_pos=vis_pos, txt_pos=txt_pos, pad_mask=pad_mask)
...
class GA(nn.Module):
    ...
    def forward(self, reg_token, vis, txt, pad_mask, query_pos=None, return_attn=False):
        ...
        for layer in self.layers:
            if return_attn:
                reg_token, vis, attn = layer(reg_token, vis, txt, None, None, pad_mask, query_pos, return_attn)
            else:
                reg_token, vis = layer(reg_token, vis, txt, None, None, pad_mask, query_pos, return_attn)
        ...
        if self.norm is not None:
            # HW, b, 512 -> b, 512, HW
            reg_token = self.norm(reg_token).permute(1, 2, 0)
            if self.return_intermediate:
                intermediate.pop()
...
class SGALayer_2(nn.Module):
    ...
    def forward(self, reg, vis, txt, vis_pos, txt_pos, pad_mask, query_pos=None, return_attn=False):
        '''
            vis: 26*26, b, 512
            txt: L, b, 512
            vis_pos: 26*26, 1, 512
        ...
        # txt-attention
        vis = self.norm2(vis)
        q = self.with_pos_embed(vis, vis_pos)
        k = self.with_pos_embed(txt, txt_pos)
        src = self.multihead_attn(q, k, value=txt)[0]
        src = self.cross_attn_norm(src)
        src = self.dropout2(src)
        vis = src + vis
        # self-attention
        n_reg = reg.size(0)
        src = torch.cat([reg, vis], dim=0)
        src = self.norm3(src)
        q = src.clone()
        k = q[n_reg:] = self.with_pos_embed(src[n_reg:], vis_pos)
        src2, attn = self.self_attn(q, k, value=src[n_reg:])
        src2 = self.self_attn_norm(src2)
...
class BridgerFormer(nn.Module):
    def __init__(self, d_model, nhead,):
        ...
        self.cross_t = CrossAttention(d_model=d_model, nhead=nhead, num_layers=2)

        self.self_v = CrossAttention(d_model=d_model, nhead=nhead, num_layers=1)
        self.self_t = SelfAttention(d_model=d_model, nhead=nhead, num_layers=1)
                
    def forward(self, vis, txt):
            
        vis, txt = self.cross_v(vis, txt), self.cross_t(txt, vis)
        
        vis = self.self_v(vis, vis)
        txt = self.self_t(txt)
