...
from .layers_6_2_4 import conv_layer, deconv_layer, InteractorT, Interactor, MLP, CrossAttention, SelfAttention, BridgerFormer
...
class Bridger_ViT_1(nn.Module):
    def __init__(self,
        ...
        for i in range(num_stages):
            ...
            self.reg_proj.append(nn.Linear(d_model*2, d_model))
    ...
    def forward(self, img, text, backbone):
        ...
        # reg_token = None
        bs = txt.size(1)  
        tgt = torch.zeros(self.n_reg, bs, self.d_model).cuda()
        ...
        for i in range(self.num_layers):
            if i not in [3,5,7,9,11]:
                ...
                cls = v[:1]
                v = v[1:, :, :]
                reg = self.reg_proj[stage_i](torch.cat([reg,cls], dim=-1))
                reg, v, t, attn = self.bridgeformer[stage_i](tgt+reg, v, t, pos_query=None, return_attn=True)
        ...
        # forward
        output = v_cls, vis_outs, txt, state, reg_tokens, attn_ls
