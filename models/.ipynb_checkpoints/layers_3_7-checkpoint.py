...
class HA1(nn.Module):
    def __init__(self,
        ...
        # aggregation
        self.aggr = conv_layer(4 * out_channels, out_channels, 1, 0)
    ...
    def forward(self, imgs, state):
        ...
        fq = torch.cat(imgs, dim=-1)
        n, bs, d = fq.size()
        h = w = int(n**0.5)
        fq = fq.permute(1,-1,0).view(bs, d, h, w)
        fq = self.aggr(fq)
...
class SGALayer_2(nn.Module):
    ...
    def forward(self, tgt, vis, txt, vis_pos, txt_pos, pad_mask, query_pos=None, return_attn=False):
        '''
            vis: 26*26, b, 512
            txt: L, b, 512
            vis_pos: 26*26, 1, 512
        ...
        k = self.with_pos_embed(txt, txt_pos)
        q = vis.clone()
        q = self.with_pos_embed(q, vis_pos)
        vis2 = self.multihead_attn(query=q,
                                   key=k,
                                   value=txt,
                                   key_padding_mask=pad_mask)[0]
        vis2 = self.cross_attn_norm(vis2)
        vis2 = self.dropout2(vis2)
        vis2 += vis
        # self-attention
        src = torch.cat([tgt, vis2], dim=0)
        src2 = self.norm3(src)
        q = src2.clone()
        q[0] = self.with_pos_embed(q[0], query_pos)
        q[1:] = self.with_pos_embed(q[1:], vis_pos)
