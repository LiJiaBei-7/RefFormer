...
from .layers_6_2_5_2 import conv_layer, deconv_layer, InteractorT, Interactor, MLP, CrossAttention, SelfAttention, QFormer 
...
class QA(nn.Module):
    def __init__(self,
        ...
        self.aux_cls = nn.ModuleList()

        self.bridger_stages = bridger_stages
        self.num_stages = len(bridger_stages)
  
        
        for i in range(self.num_stages):
            self.qformer.append(QFormer(d_model, nhead))
            ...
            self.aux_cls.append(MLP(d_model, d_txt, 2, 3))
    ...
    def forward(self, img, text, backbone):
        ...
        for i in range(self.num_layers):
            if i not in self.bridger_stages:
                vis = vis_enc.transformer.resblocks[i](vis)
                if (i+1) % 2 == 0:
                    m = int((i-1) / 2)
                    txt = txt_enc.resblocks[m](txt)
            else:
                vis = vis_enc.transformer.resblocks[i](vis)
                m = int((i-1) / 2)
                txt = txt_enc.resblocks[m](txt)                 
                ...
                tgt, v, t, attn = self.qformer[stage_i](tgt+reg, v, t, pos_query=None, return_attn=True)
        ...
        # forward
        output = vis_outs, txt, state, reg_tokens, feat_ls, attn_ls
